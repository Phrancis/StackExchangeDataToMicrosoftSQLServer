...
-- Edit this to the number of Votes XML rows (i.e. files) you want to process:
...
DECLARE _VotesXmlProcessing CURSOR FOR
    SELECT
        RowNum, 
        SiteDirectory, 
        FilePath 
...
OPEN _VotesXmlProcessing;

FETCH NEXT FROM _VotesXmlProcessing INTO @RowNum, @SiteDirectory, @FullFilePath;

WHILE @@FETCH_STATUS = 0
BEGIN
    SET @Now = GETDATE();
    EXECUTE RawDataXml.usp_LoadVotesXml @SiteDirectory, @FullFilePath;
...
CLOSE _VotesXmlProcessing;
DEALLOCATE _VotesXmlProcessing;
...
SELECT COUNT(*) AS [VotesXmlLeftToProcess] 
FROM RawDataXml.XmlProcessingQueue AS q
WHERE q.DataType = 'Votes' 
AND q.Processed = 0
AND q.SiteDirectory IN (
